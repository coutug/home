...
- `hosts/nixos-mini1`: worker host with the current `stateVersion = "25.11"`. It imports the disk layout (`disk-config.nix`), enables the `disko` module, and references the `facter.json` report so `nixos-anywhere` can build the host with the right drivers.
- `hosts/nixos-mini2`: primary host that boots the `k0s-mini` cluster as a controller+worker. It keeps `hardware.facter.reportPath` enabled and sanitizes null CPU entries from the report to avoid the `hardware/facter/virtualisation.nix` evaluation failure.
