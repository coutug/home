...
- `home/`
  ...
  - `home/config/k9s/cyberdream.nix`: mirrors the cyberdream extra for k9s and exposes a `cyberdreamSkin` through `home/programs/k9s.nix` so the UI loads the same palette.
...
- `hosts/`
  - `nixos-mini1/`: worker NixOS 25.11 host with static IPv4 on `enp6s0`, systemd-boot, and a disk layout described in `disk-config.nix` for `disko`. `nixos-anywhere` now generates `hosts/nixos-mini1/facter.json` via `nixos-facter` instead of `hardware-configuration.nix`.
  - `nixos-mini2/`: primary NixOS 25.11 host with its own static IPv4 and matching disk layout, booting the `k0s-mini` cluster as a controller+worker.
...
- `k0s/token-mini1.yaml`: SOPS secret that maps to `/etc/k0s/k0stoken` (owner `root`, mode `0400`) so the worker can join the cluster after the controller boots.
...
- Prepare the NixOS hosts: run `nixos-anywhere` from this repo with the per-host disk layout and generate each host facter report via `--generate-hardware-config nixos-facter hosts/<host>/facter.json` before running or rebuilding the system. Note: `nixos-mini2` keeps facter enabled and sanitizes null CPU entries from the report to avoid a nixpkgs facter virtualisation evaluation issue.
...
3. **nixos-mini hosts**: wire both hosts into the common module, expose metrics (Syncthing, node exporter, etc.), and keep each `disk-config.nix` in sync with future partition changes.
4. **Secrets**: document how to propagate age keys to hosts and validate rotation policies.
5. **Observability & services**: `nixos-mini2` runs `k0s` as controller+worker and `nixos-mini1` joins as worker, alongside Syncthing, Grafana, VictoriaMetrics, Vector, DNS, and Vaultwarden; keep this stack documented, expose secrets through `sops-nix`, and keep `disk-config.nix`/hardware assets in sync.
